...
export class OrdersService {
  ...
  async create(createOrderDto: CreateOrderDto) {
    ...
    for (const product of products) {
      try {
        const response = await axios.get(`http://localhost:4042/api/v1/products/product?id=${product.productId}`);
  ...
  async getUser(userId: string) {
    ...
    try {
      const response = await axios.get(`http://localhost:4041/api/v1/auth?id=${userId}`);
  ...
  async shipOrder(orderId: string) {
    ...
    if (order.status === Status.SHIPPED) { // Use enum if applicable
      throw new HttpException('Order is already Shipped', HttpStatus.BAD_REQUEST);
    }
...
// Use the enum value instead of the string
  ...
  async allCancelledOrder() {
    const order = await this.orderModel.find({status : Status.CANCELLED}).exec();
...
async update(orderId: string, updateOrderDto: UpdateOrderDto) {
  ...
  for (const updatedProduct of updateOrderDto.products) {
    try {
      ...
      const response = await axios.get(`http://localhost:4042/api/v1/products/product?id=${updatedProduct.productId}`);
      const retrievedProduct = response.data;
      ...
      throw new NotFoundException(`Product with ID ${updatedProduct.productId} not found`);
    }
  }
